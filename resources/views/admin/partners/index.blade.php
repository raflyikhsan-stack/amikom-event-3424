    ...
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse($partners as $partner)
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition group text-center relative overflow-hidden">
        
                    <div class="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <a href="{{ route('admin.partners.edit', $partner->id) }}" 
                        class="p-2 bg-amber-100 text-amber-600 rounded-xl hover:bg-amber-200 transition shadow-sm" title="Edit Partner">
                        ...
                        <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus partner ini?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-2 bg-red-100 text-red-600 rounded-xl hover:bg-red-200 transition shadow-sm" title="Hapus Partner">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <h5 class="font-bold text-slate-800 truncate px-2">{{ $partner->name }}</h5>
